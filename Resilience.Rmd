...
score_df = score_df %>% drop_na(gensini, average_norm_score)
zeroinflmodel = zeroinfl(100*gensini ~ average_norm_score | average_norm_score, data = score_df, dist = "negbin")
...
score_df = pheno_df %>% drop_na(gensini, sbp)
score_df %>% select(gensini, age, gender, Chol_mgdl, HDL_mgdl, sbp, bp_med, curr_smok, cvhx_dm)
zeroinflmodel = zeroinfl(100*gensini ~ age + gender + Chol_mgdl + HDL_mgdl + sbp + bp_med + curr_smok + cvhx_dm | age + gender + Chol_mgdl + HDL_mgdl + sbp + bp_med + curr_smok + cvhx_dm, data = score_df, dist = "negbin")
summary(zeroinflmodel)
...
ggplot(score_df, aes(x = age, y = log(gensini+1), color = consensus_class)) +
  geom_point() +
  scale_color_manual(values = c("resilient" = "green", "reference" = "blue", "susceptible" = "red", "ignore" = "grey50")) +
  theme_bw() +
