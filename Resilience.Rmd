...
```{r}
topTable(constrast_fit)
selected_rows <- subset(topTable(constrast_fit, n=100), adj.P.Val < 0.05)
selected_metabolites <- rownames(selected_rows)
selected_columns <- c("ID", selected_metabolites)
filtered_df <- prot_patient_df[, colnames(prot_patient_df) %in% selected_columns]
merged_df <- merge(filtered_df, dat_FRS[, c("ID", "LDL")], by = "ID")


for(metabolite in selected_metabolites) {
  plot_data <- merged_df[, c("ID", metabolite, "LDL")]
  p <- ggplot(plot_data, aes_string(x=metabolite, y="LDL")) +
    geom_point() +
    geom_smooth(method="lm", se=FALSE) +
    ggtitle(paste("Dot Plot of", metabolite, "vs LDL"))
  print(p)
  
  correlation <- cor.test(plot_data[[metabolite]], plot_data[["LDL"]])
  cat("Correlation between", metabolite, "and LDL is:", correlation$estimate, "\n")
  cat("P-value is:", correlation$p.value, "\n\n")
}
```
...
# Now run all the individual analysis of each metabolites
```


### Score Binning (Alternative Approach)
...
# 1. Bin subjects into 10 bins by ascvd_10y_frs risk score
