...
library(h2o)
features = as.h2o(X_train) %>% h2o.na_omit()
...
metab_lipid_prot_cytof_merged_df = inner_join(exp1_df, exp2_df, by=c('primary', 'colname', 'age', 'smurfs', 'cacs', 'cacs_pct', 'gensini')) %>%
  inner_join(exp3_df, by=c('primary', 'age', 'smurfs', 'cacs', 'cacs_pct', 'gensini')) %>%
  inner_join(exp4_df, by=c('primary', 'colname', 'age', 'smurfs', 'cacs', 'cacs_pct', 'gensini'))


metab_lipid_prot_merged_df = inner_join(exp1_df, exp2_df, by=c('primary', 'colname', 'age', 'smurfs', 'cacs', 'cacs_pct', 'gensini')) %>%
  inner_join(exp3_df, by=c('primary', 'age', 'smurfs', 'cacs', 'cacs_pct', 'gensini'))
...
With 251 final variables
R2: 8.332503	RMSE: 0.5935032	MAE: 6.395322	
...
data.frame(
  RMSE = RMSE(pred$predictions, y_test),
  Rsquare = R2(pred$predictions, y_test),
  MAE = MAE(pred$predictions, y_test)
)
```
...
graph_df = cbind(metab_lipid_prot_merged_df[-train,],y_test, pred, age_accl) %>%
  select(primary, age, smurfs, cacs, cacs_pct, gensini, y_test, pred, age_accl)
...
library(plotly)
in# People who have low CACS score but high SMURFs
graph = graph_df %>%
  mutate(cac_present = case_when(
    cacs_pct>0 ~ "present",
    cacs_pct==0 ~ "absent"
