...
long_score_df$stud_class = long_score_df$consensus_class
...
long_score_df$harm_class = score_df$consensus_class
long_score_df[!long_score_df$harm_class == long_score_df$stud_class,] %>%
...
filtered_graph_df = filtered_graph_df %>%
  mutate(cacs_bool  = case_when(
    cacs != 0 ~ "CACS_present",
    cacs == 0 ~ "CACS_absent"
  )) 

relevel(filtered_graph_df$cacs_bool, "CACS_absent")
...
# Contrasts
CM = makeContrasts(cacs_boolCACS_present, levels = design)
constrast_fit = contrasts.fit(efit, contrast = CM)
constrast_fit = eBayes(constrast_fit)
