...
bootstrap_metric = function(X, y, model, n) {
  ...
  a = metric
  df = cbind(
        rbind(RMSE = RMSE(pred, y), Rsquare = R2(pred, y), MAE = MAE(pred, y)),
        rbind(
...
graph_df = cbind(metab_lipid_prot_merged_df[-train,],y_test, pred, age_accl) %>%
  select(primary, age, smurfs, cacs, cacs_pct, gensini, y_test, pred, age_accl, cvd_FRS, chd_FRS) %>%
...
# Graph Resilience vs Age Acceleration
graph_df %>%
  filter(cacsplus == "actionable") %>%
  filter(is.na(cvd_FRS) == FALSE) %>%
  mutate(resilience_orthogonals = resilience_orthogonals) %>%
  ggplot() +
  aes(x = age_accl, y = resilience_orthogonals) +
  geom_point() +
  stat_smooth(method='lm', col='red') +
  labs(
    x = "Age Acceleration",
