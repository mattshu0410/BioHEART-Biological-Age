...
score_df = pheno_df %>%
  select(ID, cacs, cacs_pct, ascvd_10y_frs, ascvd_10y_accaha, chd_10y_mesa, SCORE2)
...
```{r}
# Calculate residuals of linear model regressing CACS %tile on score
...
# Weird unresolved issue where people with 0 risk but high CACS percentile
lm = lm(score_df$cacs_pct ~ score_df$ascvd_10y_accaha)
summary(lm)
residuals(lm)

ggplot() +
  aes(x = score_df$ascvd_10y_accaha, y = score_df$cacs_pct) +
  geom_point() +
  geom_smooth(method = 'lm')

# Calculates all residuals for all scores
score_df = score_df %>%
  mutate(
    res_ascvd_10y_frs = calculate_residuals(cacs_pct, ascvd_10y_frs),
    res_ascvd_10y_accaha = calculate_residuals(cacs_pct, ascvd_10y_accaha),
    res_chd_10y_mesa = calculate_residuals(cacs_pct, chd_10y_mesa),
    res_SCORE2 = calculate_residuals(cacs_pct, SCORE2)
  )
...
calculate_classes = function(residuals, reference_cutoff = 0.3, top_vs_bottom_quantile = FALSE) {
...
score_df = score_df %>%
  mutate(
    class_ascvd_10y_frs = calculate_classes(res_ascvd_10y_frs, top_vs_bottom_quantile = TRUE),
    class_ascvd_10y_accaha = calculate_classes(res_ascvd_10y_accaha, top_vs_bottom_quantile = TRUE),
    class_chd_10y_mesa = calculate_classes(res_chd_10y_mesa, top_vs_bottom_quantile = TRUE),
    class_SCORE2 = calculate_classes(res_SCORE2, top_vs_bottom_quantile = TRUE)
  )


score_df %>%
  ggplot() +
    aes(x = ascvd_10y_frs, y = cacs_pct, color = class_ascvd_10y_frs, group = 1) +
    geom_point() +
    geom_smooth(method = 'lm') +
    labs(y = "CACS (%tile)",
         title = "Lower 30%tile vs Upper 30%tile")
...
dim(lm_df)
dim(design)
design = model.matrix(~ consensus_class + gender + age, graph_df)
